...
def constant_deriv(t: float, x: np.ndarray | list):
    return np.zeros_like(x)

def rk4_test_constant():
...
def slope_2(t: float, x: np.ndarray | list):
    return np.array([2,4])

def rk4_test_constant_slope():
    x0 = [1,2]
    dt = 2
    t = 0
...
def e_decay_deriv(t: float, x: np.ndarray | list):
    return -x

def rk4_test_exponential_decay():
...
def rk4_test_exponential_decay_multistep():
    y0 = np.array([4,10]) # Starts at y=4
    dt = 0.005
    t_max = 10
    t = 0
...
def sine_deriv(t: float, x: np.ndarray | list):
    return np.cos(t)
...
def logistic_deriv(t: float, x: np.ndarray | list):
    rate = 1
    k = 10 # asymptote
    return  rate * x*(1-x/k)

def rk4_test_logistic():
